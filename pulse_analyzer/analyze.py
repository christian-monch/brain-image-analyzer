...
from differences import plot_difference_graph


analysis_modules = {
    "difference": plot_difference_graph
}


argument_parser = ArgumentParser(
    prog='pulse_analyzer',
    description='Analyzes pulse events in image stacks. Provides a number of experimental analysis modules: '
                f'{", ".join(analysis_modules.keys())}.',
    epilog='for more information see: https://github.com/christian-monch/brain-image-analyzer')

argument_parser.add_argument(
    'module',
    help=f'module that should be used, one of: {", ".join(analysis_modules.keys())}'
)
...
def find_all_pulses_image(image_stack):
    ...
    for r in range(rows):
        for c in range(columns):
            pulse_info = find_pulses(image_stack, r, c, **(dataclasses.asdict(pulse_parameter)))
    ...
    if cluster_parameter.pulse_value_normalize is True:
...
def cluster_analysis(image_stack: np.ndarray,
                     ...
                     show_images=True, normalize=100):
